...
public:
    ...
    void chargeAccount(double amount); // اعلان متد، پیاده‌سازی در ادامه
        ...
        if (prodFile) {
            ...
            while (getline(prodFile, line)) {
                ...
                if (pos1 != string::npos && pos2 != string::npos && pos3 != string::npos) {
                    string name = unescapeCommas(line.substr(0, pos1));
...
public:
    ...
    // متد جدید برای شارژ حساب مشتری
    void chargeCustomerAccount(Customer* customer, double amount) {
        if (amount < 0) {
        ...
        saveUsers(); // ذخیره‌سازی فوری کاربران
        cout << endl;
    ...
    void addProduct(const Product& product) {
        ...
        saveProduct(); // ذخیره‌سازی فوری محصولات
        cout << endl;
    ...
    void removeProduct(const string& productName) {
        auto it = remove_if(products.begin(), products.end(), [&productName](const Product& Pro) {
            return Pro.GetName() == productName;
            });
...
// پیاده‌سازی متد chargeAccount در کلاس Customer
void Customer::chargeAccount(double amount) {
    PetShopSystem* system = PetShopSystem::GetInstance();
    system->chargeCustomerAccount(this, amount);
}
            ...
            for (const auto& Product : system->GetProduct()) {
                if (Product.GetName() == productName) {
                    ...
                    if (makePurchase(Product.GetPrice())) {
                        ...
                        system->saveUsers(); // ذخیره‌سازی فوری کاربران
                        cout << endl;
