...
private:
    ...
    void loadingAnimation(const string& message, int durationMs = 2000) {
    ...
    void spinnerAnimation(const string& message, int spins = 4) {
        const char spinner[] = { '|', '/', '-', '\\' };
...
============================================================
        ...
        loadingAnimation("Initializing", 2500);
    }
    ...
    void loadReservations() {
        ...
        if (file.is_open()) {
            ...
            ofstream createFile(RESERVATIONS_FILE);
            createFile.close();
    ...
    void searchReservation() {
        ...
        for (const auto& res : reservations) {
            string guestName = res.guestName;
            replace(guestName.begin(), guestName.end(), '_', ' ');
            if (guestName == input || (roomNum != 0 && res.roomNumber == roomNum)) {
...
public:
    ...
    void reserveRoom() {
        ...
        if (it != rooms.end()) {
            ...
            while (!validateDate(checkIn)) {
                cout << RED << "Invalid date (use YYYY-MM-DD). Try again: " << RESET;
                getline(cin, checkIn);
                if (checkIn == "b" || checkIn == "B") {
                    clearScreen();
                    return;
            ...
            if (confirm == "y" || confirm == "Y") {
                ...
                saveRooms();
                saveReservations();
                loadingAnimation("Confirming reservation", 1500);
        ...
        if (roomIt != rooms.end()) {
            ...
            if (resIt != reservations.end()) {
                ...
                if (confirm == "y" || confirm == "Y") {
                    ...
                    saveRooms();
                    saveReservations();
                    loadingAnimation("Processing check-out", 1500);
    ...
    void addNewRoom() {
        ...
        if (confirm == "y" || confirm == "Y") {
            ...
            saveRooms();
            loadingAnimation("Adding new room", 1500);
